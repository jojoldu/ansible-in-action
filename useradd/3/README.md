...
등의 방법밖에 없습니다.  
그래서 이런 설정들까지 파일로 관리해보겠습니다.  
  
현재 플레이북이 있는 디렉토리 ```~/ansible-useradd```에 ```group_vars``` 디렉토리를 생성합니다.  
(보편적인 인벤토리 디렉토리 이름입니다.)  
  
```bash
mkdir groups_vars
```

그리고 아무런 확장자가 없는 ```all``` 파일을 생성합니다.
...
그리고 all 파일에 그동안 전역 설정으로 사용된 ```hosts```의 내용과 ```-u ec2-user```로 사용된 코드를 등록합니다.
